...
class SCUBEDataloader(Dataloader):
    ...
    def __init__(self, load_path: str, file_name: str, dtype: pt.dtype = pt.float32):
        ...
        # later for writing or other operations
        ...
        if self._field_names is None:
            ...
            with File(join(self._load_path, self._file_name), "r") as f:
                self._field_names = {
                    k: [
                        f.split("_")[0]
                        for f in f[f"{DATA}/{k}"].keys()
                        if f.endswith("center")
                    ]
    ...
    def _compute_cell_area(self) -> None:
        ...
        self._weights = (
            1
            / pow(2, self._n_dimensions)
            * pow(self._size_initial_cell / pow(2, self.levels), self._n_dimensions)
        ).squeeze()

    def load_snapshot(
        self,
        field_name: Union[List[str], str],
        write_times: Union[str, List[str]] = None,
        ...
        :return: either a data matrix containing the snapshots as: [N_cells, N_dimensions, N_snapshots] (vector field) and
        ...
        # create file object and empty list for storing the data matrices
        ...
        for f in field_name:
            ...
            if len(shape) == 1:
                _data_matrix = pt.zeros(
                    (self._n_cells, len(write_times)), dtype=self._dtype
                )
            else:
                _data_matrix = pt.zeros(
                    (shape[0], shape[1], len(write_times)), dtype=self._dtype
                )

            # assemble the data matrix for the given time steps
            for i, k in enumerate(write_times):
                if len(shape) == 1:
                    _data_matrix[:, i] = pt.from_numpy(
                        _file.get(f"{DATA}/{k}/{f}_center")[()]
                    )
                else:
                    _data_matrix[:, :, i] = pt.from_numpy(
                        _file.get(f"{DATA}/{k}/{f}_center")[()]
