...
class CSVDataloader(Dataloader):
    ...
    def from_davis(
        ...
        if not set(DAVIS_KEYS[VERTEX_KEY]).issubset(set(column_names)):
            logger.warning("DaVis files do not contain vertices")
        if not set([DAVIS_KEYS[WEIGHT_KEY]]).issubset(set(column_names)):
            logger.warning("DaVis files do not contain isValid mask")
        read_options = {
    ...
    def from_foam_surface(
        cls, path: str, file_name: str, header: int = 1, dtype: pt.dtype = DEFAULT_DTYPE
    ):
        ...
        *surfaceFormat* is set to *raw*. The time series data are stored in
        ...
        times = sorted([folder.split(sep)[-1] for folder in folders], key=float)
        ...
        weight = (
            FOAM_SURFACE_KEYS[WEIGHT_KEY]
            if all([key in column_names for key in FOAM_SURFACE_KEYS[WEIGHT_KEY]])
            else None
        )
        read_options = {
            ...
            FIELD_KEY: [
                ...
                for col in column_names
                if col
                not in FOAM_SURFACE_KEYS[VERTEX_KEY] + FOAM_SURFACE_KEYS[WEIGHT_KEY]
            ],
        }
    ...
    def _determine_write_times(self) -> List[str]:
        ...
        if self._time_folders:
            ...
            return sorted([folder.split(sep)[-1] for folder in folders], key=float)
        else:
    ...
    def field_names(self) -> Dict[str, List[str]]:
        return dict({self.write_times[0]: self._read_options[FIELD_KEY]})
    ...
    def weights(self) -> pt.Tensor:
        ...
        if weight_key is not None:
            if isinstance(weight_key, list):
                return pt.tensor(snapshot[weight_key].values, dtype=self._dtype).norm(
                    dim=1
                )
            else:
                return pt.tensor(snapshot[weight_key].values, dtype=self._dtype)
        else:
